Backbone.GUI = (function(GUI) {
  ...
  GUI.Dropdown = GUI.RadioButtons.extend({
    ...
    changeInput: function(e) {
      ...
      // if they pass in `property`,
      // we want to change the property of the model
      if (this.options.property) {
        this.model.set(this.options.property, val);
      }

      // they passed in a callback
      // send callback new value
    ...
    render: function() {
      ...
      _.each(this.options.options, function(opt) {
        ...
        // options was a list of strings
        if (_.isString(opt)) {
          name = opt;

        // options was a list of models
        } else {
          name = opt.get('name');
        }
  ...
  GUI.VerticalSlider = Backbone.GUI.Component.extend({
    ...
    onSlide: function(e) {
      ...
      var model = this.model,
        ...
        rel_y_px = (bottom - e.clientY) - $(document).scrollTop(),         // px from bottom user clicked
